...
def cargarBDPersonalidades():
    ...
    f=open("personalidades.txt","r")
    diccioPersonali={}
    for linea in f.readlines():
        if linea[0]=="*":
            personalidadSecundaria.append((linea[:-6],linea[-5:-1]))
        if linea[0]=="-":
            diccioPersonali.update({linea[:-1]:personalidadSecundaria})
            personalidadSecundaria=[]
    return diccioPersonali

print(cargarBDPersonalidades())
